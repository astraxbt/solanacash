...
export class ShieldedPoolMerkleTree {
  ...
  /** Export all leaves for persistence */
  exportLeaves(): string[] {
    return this.leaves.map((l) => "0x" + l.toString(16).padStart(64, "0"));
  }

  /** Import leaves from persistence */
  importLeaves(hexLeaves: string[]): void {
    this.leaves = hexLeaves.map((h) => BigInt(h));
  }
}
